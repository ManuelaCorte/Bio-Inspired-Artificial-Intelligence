...
def main(
    eval_fn: Callable[[NDArray[np.float64]], float],
    NO_BINS: int,
    MAX_ITEMS_BIN: int,
    BUDGET: int,
    BATCH_SIZE: int,
    ...
    logger = algorithms.TQDMAlgorithmLogger(
        algo, log_base_path="results/ex2/" + str(seed)
    )

    # Run illumination process !
    with ParallelismManager("none") as pMgr:
        algo.optimise(
            eval_fn, executor=pMgr.executor, batch_mode=False
        )  # Disable batch_mode (steady-state mode) to ask/tell new individuals without waiting the completion of each batch
