...
class SortingNetwork(list):
    ...
    def assess(self, cases=None):
        ...
        misses = 0
        ordered = [
            [0] * (self.dimension - i) + [1] * i for i in range(self.dimension + 1)
        ]
...
def main(seed, creator, htoolbox, ptoolbox, config):
    ...
    config["H_TRNMT_SIZE"]  # tournament size for hosts
    config["P_TRNMT_SIZE"]  # tournament size for parasites
    config["P_NUM_SEQ"]  # number of shuffled sequences for each parasite
    random.seed(seed)
