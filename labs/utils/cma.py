...
it depends on collections.abc.MutableMapping, since version 0.91.01).
...
# from __future__ import collections.abc.MutableMapping # does not exist in future, otherwise 2.5 would work
...
__version__ = (
    "0.92.05 $Revision: 3322 $ $Date: 2012-11-22 18:05:10 +0100 (Thu, 22 Nov 2012) $"
)
...
from numpy import (
    ...
    sqrt,
    sum,
)  # to access the built-in sum fct:  __builtins__.sum or del sum removes the imported sum and recovers the shadowed
...
use_sent_solutions = (
    True  # 5-30% CPU slower, particularly for large lambda, will be mandatory soon
)
        ...
        if len(self) > max_len:
            for k in list(self.keys()):
                if self[k]["iteration"] < min_iter:
                    del self[
                        k
                    ]  # only deletes one item with k as key, should delete all?
    ...
    def update(self, function_values, es, bounds=None):
        ...
        varis = es.sigma**2 * array(
            ...
            # scalar case
                ...
                es.C
                if np.isscalar(es.C[0])
                # diagonal matrix case
                else [es.C[i][i] for i in xrange(N)]
            )
        )  # full matrix case

        # dmean = (es.mean - es.gp.into_bounds(es.mean)) / varis**0.5
        dmean = (
            es.mean - es.gp.geno(es.gp.into_bounds(es.gp.pheno(es.mean)))
        ) / varis**0.5
        ...
        if self.weights_initialized:
            ...
            if 1 < 3:  # this is better, around a factor of two
                ...
                self.gamma *= exp((edist > 0) * np.tanh(edist / 3) / 2.0) ** damp
                ...
                self.gamma *= exp(np.max([N * [0], 0.3 - r], axis=0)) ** min(
                    1, es.sp.mueff / 10 / N
                )
...
class GenoPheno(object):
    ...
    def __init__(
        self, dim, scaling=None, typical_x=None, bounds=None, fixed_values=None, tf=None
    ):
        ...
        if bounds:
            ...
            for i in (0, 1):
                if bounds[i] is not None:
                    bounds[i] = array(
                        dim * [bounds[i]]
                        if np.isscalar(bounds[i])
                        else [b for b in bounds[i]]
                    )
        ...
        if (
            self.scales == 1
            and self.typical_x == 0
            and self.bounds in (None, [None, None])
...
class CMAEvolutionStrategy(OOOptimizer):
    ...
    *pairwise selection* (where at most one of the two mirrors contribute to the
    ...
    def stop(self, check=True):
        ...
        if (
            check
            and self.countiter > 0
            and self.opts["termination_callback"]
            and self.opts["termination_callback"]
            != str(self.opts["termination_callback"])
        ):
            self.callbackstop = self.opts["termination_callback"](self)

        return self.stopdict(
            self if check else None
    ...
    def __init__(self, x0, sigma0, inopts={}):
        ...
        opts = Options(
            inopts
        ).complement()  # Options() == fmin([],[]) == defaultOptions()
        ...
        self.fmean = (
            np.nan
        )  # TODO name should change? prints nan (OK with matlab&octave)
        self.fmean_noise_free = 0.0  # for output only
        ...
        self.countiter = 0
        self.countevals = (
            max((0, opts["verb_append"]))
            if type(opts["verb_append"]) is not bool
            else 0
        )
        self.ps = np.zeros(N)
        self.pc = np.zeros(N)
        ...
        if self.opts["CMA_diagonal"]:  # is True or > 0
            ...
            self.B = np.eye(
                N
            )  # identity(N), do not from matlib import *, as eye is a matrix there
        ...
        if not opts["seed"]:
            ...
            opts["seed"] = (
                1e5 * np.random.rand() + six_decimals + 1e5 * (time.time() % 1)
            )
        ...
        self.const.chiN = N**0.5 * (
            1 - 1.0 / (4.0 * N) + 1.0 / (21.0 * N**2)
        ...
        if opts["verb_disp"] > 0:
            ...
            if opts["CMA_diagonal"] and self.sp.CMA_on:
                ...
                if opts["CMA_diagonal"] is not True:
                    ...
                    s += " iterations"
                    s += (
                        " (1/ccov=" + str(round(1.0 / (self.sp.c1 + self.sp.cmu))) + ")"
                    )
    ...
    def ask(self, number=None, xmean=None, sigma_fac=1):
        ...
        pop_pheno = [
            self.gp.pheno(x, copy=True, bounds=self.gp.bounds) for x in pop_geno
        ]

        if (
            not self.gp.isidentity or use_sent_solutions
        ):  # costs 25% in CPU performance with N,lambda=20,200
            # archive returned solutions, first clean up archive
            if self.countiter % 30 / self.popsize**0.5 < 1:
                self.sent_solutions.truncate(
                    0, self.countiter - 1 - 3 * self.N / self.popsize**0.5
                )
            # insert solutions
            for i in xrange(len(pop_geno)):
                self.sent_solutions[pop_pheno[i]] = {
                    "geno": pop_geno[i],
                    "pheno": pop_pheno[i],
    ...
    def ask_geno(self, number=None, xmean=None, sigma_fac=1):
        ...
        if self.sp.CMA_on and (
            (
                self.opts["updatecovwait"] is None
                and self.countiter
                >= self.itereigenupdated
                + 1.0 / (self.sp.c1 + self.sp.cmu) / self.N / 10
            )
            or (
                self.opts["updatecovwait"] is not None
                and self.countiter > self.itereigenupdated + self.opts["updatecovwait"]
            )
        ):
            self.updateBD()
        ...
        if 11 < 3:  # mutate along the principal axes only
            perm = np.random.permutation(
                self.N
            )  # indices for mutated principal component
    ...
    def ask_and_eval(
        ...
        for k in xrange(int(popsize)):
            ...
            while f in (np.nan, None):  # rejection sampling
                ...
                if (
                    11 < 3 and self.opts["vv"] and nreject < 2
                ):  # trying out negative C-update as constraints handling
                    ...
                    if (
                        1 < 3
                        and sum([(9 + i + 1) * x[i] for i in xrange(self.N)]) > 50e3
                    ):
                        self.constraints_paths[0] = (
                            0.9 * self.constraints_paths[0]
                            + 0.1 * (x - self.mean) / self.sigma
                        )
                        self.C -= (self.opts["vv"] / self.N) * np.outer(
                            self.constraints_paths[0], self.constraints_paths[0]
                        )

                f = func(x, *args)
                if f not in (np.nan, None) and evaluations > 1:
                    f = aggregation(
                        [f] + [func(x, *args) for _i in xrange(int(evaluations - 1))]
                    )
                if nreject + 1 % 1000 == 0:
                    print(
    ...
    def tell(self, solutions, function_values, check_points=None, copy=False):
        ...
        if lam != array(function_values).shape[0]:
            raise _Error(
                "for each candidate solution " + "a function value must be provided"
            )
        ...
        sp = self.sp
        if (
            11 < 3 and lam != sp.popsize
        ):  # turned off, because mu should stay constant, still not desastrous
        ...
        self.best.update(
            solutions, self.sent_solutions, function_values, self.countevals
        )

        flgseparable = (
            self.opts["CMA_diagonal"] is True
        ...
        fit.bndpen = self.boundPenalty.update(function_values, self)(
        ...
        self.out["recent_x"] = array(
            solutions[fit.idx[0]]
        )  # TODO: change in a data structure(?) and use current as identify
        ...
        if (self.countiter % 5) == 0:  # 20 percent of 1e5 gen.
            ...
            fit.histmedian.insert(
                0,
                np.median(fit.fit) if len(fit.fit) < 21 else fit.fit[self.popsize // 2],
            )
        ...
        for s in solutions:  # use phenotype before Solution.repair()
            if use_sent_solutions:
                x = self.sent_solutions.pop(
                    s, None
                )  # 12.7s vs 11.3s with N,lambda=20,200
                if x is not None:
                    pop.append(x["geno"])
                    ...
                    pop.append(
                        self.gp.geno(s, copy=copy)
                    )  # cannot recover the original genotype with boundary handling
                    if check_points in (None, True, 1):
                        self.repair_genotype(
                            pop[-1]
                        )  # necessary if pop[-1] was changed or injected by the user.
        ...
        if (
            11 < 3 and self.opts["vv"] and check_points
        ):  # TODO: check_points might be an index-list
            cmean = self.sp.cmean / min(
                1,
                (sqrt(self.opts["vv"] * N) + 2)
                / (  # abuse of cmean
        ...
        if 11 < 3:  # plot length of mean - mold
            self.more_to_write += [
                sqrt(sp.mueff)
                * sum(((1.0 / self.D) * dot(self.B.T, self.mean - mold)) ** 2) ** 0.5
                / self.sigma
                / sqrt(N)
                / cmean
            ]
        ...
        self.ps = (1 - sp.cs) * self.ps + (
            ...
            self.B, (1.0 / self.D) * dot(self.B.T, (self.mean - mold) / self.sigma_vec)
        )

        # "hsig", correction with self.countiter seems not necessary, also pc starts with zero
        hsig = sum(self.ps**2) / (
            1 - (1 - sp.cs) ** (2 * self.countiter)
        ) / self.N < 2 + 4.0 / (N + 1)
        if 11 < 3:
            # hsig = 1
            # sp.cc = 4 / (N + 4)
        ...
        self.pc = (1 - cc) * self.pc + hsig * (
            sqrt(cc * (2 - cc) * sp.mueff) / self.sigma / cmean
            ...
            if not flgseparable:
                ...
                if 11 < 3:  # ?3 to 5 times slower??
                    ...
                    for k in xrange(sp.mu):
                        z = pop[k] - mold
                        Z += np.outer(
                            (cmu * sp.weights[k] / (self.sigma * self.sigma_vec) ** 2)
                            * z,
                            z,
                        )
        ...
        if 1 < 3:  #
            ...
            self.sigma *= sigma_fac * np.exp(
                (min((1000, (sp.cs / sp.damps / 2) * (sum(self.ps**2) / N - 1))))
            )
        if 11 < 3:
            ...
            self.sigma *= np.exp(
                self.sp.mueff**0.5 * dot(self.sp.weights, lengths / self.const.chiN - 1)
            ) ** (2 / (N + 1))
        ...
        # N = self.N
        mindx = (
            eval(self.opts["mindx"])
            if type(self.opts["mindx"]) == type("")
        ...
        if (
            11 < 3
            and self.opts["verb_log"] > 0
            and (self.countiter < 4 or self.countiter % self.opts["verb_log"] == 0)
        ):
            # this assumes that two logger with the same name access the same data!
            CMADataLogger(self.opts["verb_filenameprefix"]).register(
                self, append=True
            ).add()
    ...
    def updateBD(self):
        ...
        if type(self.opts["CMA_eigenmethod"]) is type(1):
            print(
        ...
        if 11 < 3 and any(
            abs(sum(self.B[:, 0 : self.N - 1] * self.B[:, 1:], 0)) > 1e-6
        ):
    ...
    def update_exponential(self, Z, eta, BDpair=None):
        ...
        self.C = dot(
            Cs,
            dot(
                Mh.expms(eta * dot(Csi, dot(Z, Csi)), self.opts["CMA_eigenmethod"]), Cs
            ),
        )
    ...
    def disp(self, modulo=None):  # TODO: rather assign opt['verb_disp'] as default?
        ...
        if modulo:
            ...
            if self.countiter > 0 and (
                self.stop() or self.countiter < 4 or self.countiter % modulo < 1
            ):
...
class Options(dict):
    ...
    def versatileOptions():
        ...
        return tuple(
            sorted(
                i[0] for i in list(Options.defaults().items()) if i[1].find(" #v ") > 0
            )
        )
        ...
        if not unchecked:
            for key in list(self.keys()):
                if key not in Options.defaults():
                    print(
                        "Warning in cma.Options.__init__(): invalid key ``"
                        + str(key)
                        + "`` popped"
                    )
                    self.pop(key)
...
class CMAParameters(object):
    ...
    def set(self, opts, popsize=None, ccovfac=1, verbose=True):
        ...
        def conedf(df, mu, N):
            ...
            return 1.0 / (df + 2.0 * sqrt(df) + float(mu) / N)

        def cmudf(df, mu, alphamu):
            """used for computing separable learning rate"""
            return (alphamu + mu - 2.0 + 1.0 / mu) / (df + 4.0 * sqrt(df) + mu / 2.0)
        ...
        if popsize:
            ...
            popsize = opts.evalall({"N": N})[
                "popsize"
            ]  # the default popsize is computed in Options()
        sp.popsize = popsize
        ...
        if sp.mu > sp.popsize - 2 * sp.lam_mirr + 1:
            print(
                "WARNING: pairwise selection is not implemented, therefore "
        ...
        sp.c1 = (
            ccovfac * min(1, sp.popsize / 6) * cone((N**2 + N) / 2, sp.mueff, N)
        )  # 2. / ((N+1.3)**2 + sp.mucov)
        ...
        if opts["CMA_rankmu"] != 0:  # also empty
            sp.cmu = min(
                1 - sp.c1, ccovfac * cmu((N**2 + N) / 2, sp.mueff, sp.rankmualpha)
            )
            sp.cmu_sep = min(
                1 - sp.c1_sep, ccovfac * cmudf(N, sp.mueff, sp.rankmualpha)
            )
        else:
            sp.cmu = sp.cmu_sep = 0
        ...
        if 11 < 3:
            ...
            sp.damps = (
                0.3
                + 2
                * max([sp.mueff / sp.popsize, ((sp.mueff - 1) / (N + 1)) ** 0.5 - 1])
                + sp.cs
            )
        if 11 < 3:
        ...
        if 11 < 3:
            sp.damps = (
                10 * sp.damps
            )  # 1e99 # (1 + 2*max(0,sqrt((sp.mueff-1)/(N+1))-1)) + sp.cs;
...
class CMAStopDict(dict):
    ...
    def _update(self, es):
        ...
        self._addstop("ftarget", es.best.f < opts["ftarget"])
        # maxiter, maxfevals: generic criteria
        self._addstop("maxfevals", es.countevals - 1 >= opts["maxfevals"])
        self._addstop("maxiter", es.countiter >= opts["maxiter"])
        # tolx, tolfacupx: generic criteria
        # tolfun, tolfunhist (CEC:tolfun includes hist)
        self._addstop(
            "tolx",
            all([es.sigma * xi < opts["tolx"] for xi in es.pc])
            and all([es.sigma * xi < opts["tolx"] for xi in sqrt(es.dC)]),
        )
        self._addstop(
            "tolfacupx",
            any([
        ...
        self._addstop(
            "tolfun",
            es.fit.fit[-1] - es.fit.fit[0] < opts["tolfun"]
            and max(es.fit.hist) - min(es.fit.hist) < opts["tolfun"],
        )
        self._addstop(
            "tolfunhist",
            len(es.fit.hist) > 9
            and max(es.fit.hist) - min(es.fit.hist) < opts["tolfunhist"],
        )
        ...
        l = int(max(opts["tolstagnation"] / 5.0 / 2, len(es.fit.histbest) / 10))
        ...
        if 11 < 3:  #
            print(
                ...
                (
                    opts["tolstagnation"],
                    es.countiter > N * (5 + 100 / es.popsize),
        ...
        self._addstop(
            "tolstagnation",  # leads sometimes early stop on ftablet, fcigtab, N>=50?
            1 < 3
            and opts["tolstagnation"]
            and es.countiter > N * (5 + 100 / es.popsize)
            and len(es.fit.histbest) > 100
            and 2 * l < len(es.fit.histbest)
            and np.median(es.fit.histmedian[:l])
            >= np.median(es.fit.histmedian[l : 2 * l])
            and np.median(es.fit.histbest[:l]) >= np.median(es.fit.histbest[l : 2 * l]),
        )
        ...
        self._addstop(
            "tolupsigma",
            opts["tolupsigma"]
            and es.sigma / es.sigma0 / np.max(es.D) > opts["tolupsigma"],
        )
        ...
        if 1 < 3:
            ...
            if opts["CMA_diagonal"] is not True and es.countiter > opts["CMA_diagonal"]:
                ...
                self._addstop(
                    "noeffectaxis",
                    sum(es.mean == es.mean + 0.1 * es.sigma * es.D[i] * es.B[:, i])
                    == N,
                )
            self._addstop("conditioncov", es.D[-1] > 1e7 * es.D[0], 1e14)  # TODO
        ...
        if len(self):
            self._addstop(
                ...
                len(es.fit.hist) > 9 and max(es.fit.hist) == min(es.fit.hist),
            )
...
class BaseDataLogger2(DerivedDictBase):
    ...
    def data(self):
        ...
        print(
            "method BaseDataLogger.data() is not implemented, to be done in subclass "
            + str(type(self))
        )
...
class CMADataLogger(BaseDataLogger):  # might become a dict at some point
    ...
    def __init__(self, name_prefix=default_prefix, modulo=1, append=False):
        ...
        self.append = append
        self.counter = (
            0  # number of calls of add, should initial value depend on `append`?
        )
    ...
    def initialize(self, modulo=None):
        ...
        try:
            f = open(fn, "w")
            f.write(
                '% # columns=["iteration, evaluation, sigma, void, void, '
        ...
        try:
            with open(fn, "w") as f:
                f.write("% # iter+eval+sigma+0+fitness+xbest, " + strseedtime + "\n")
        except (IOError, OSError):
            print("could not open/write file " + fn)
    ...
    def add(
        self, es=None, more_data=[], modulo=None
        ...
        if type(es) is not CMAEvolutionStrategy:  # not necessary
            print(
                "WARNING: <type 'CMAEvolutionStrategy'> expected, found "
                + str(type(es))
                + " in method CMADataLogger.add"
            )
        evals = es.countevals
        ...
        try:
            ...
            with open(fn, "a") as f:
                if iteration < 1:  # before first iteration
                    f.write("0 0 0 0 0 " + " ".join(map(str, xmean)) + "\n")
                else:
                    f.write(
                        str(iteration)
                        + " "
    ...
    def plot(
        ...
        iabscissa=1,
        iteridx=None,
        plot_mean=True,  # TODO: plot_mean default should be False
        foffset=1e-19,
        x_opt=None,
        fontsize=10,
    ):
        ...
        if fontsize and pylab.rcParams["font.size"] != fontsize:
            print(
                "global variable pylab.rcParams['font.size'] set (from "
                + str(pylab.rcParams["font.size"])
                + ") to "
                + str(fontsize)
            )
            pylab.rcParams["font.size"] = (
                fontsize  # subtracted in the end, but return can happen inbetween
            )
        ...
        if dat.x.shape[1] < 100:
            ...
            idx2 = np.argsort(idx)
            if x_opt is None:
                dat.x[-1, 5 + idx] = np.linspace(
                    np.min(dat.x[:, 5:]), np.max(dat.x[:, 5:]), dat.x.shape[1] - 5
                )
            else:
                dat.x[-1, 5 + idx] = np.logspace(
                    np.log10(np.min(abs(dat.x[:, 5:]))),
                    np.log10(np.max(abs(dat.x[:, 5:]))),
                    dat.x.shape[1] - 5,
                )
        else:
            minxend = 0
        ...
        if dat.x.shape[1] < 100:
            ...
            if x_opt is not None:
                semilogy(
                    [dat.x[-1, iabscissa], ax[1]], [abs(dat.x[-1, 5:]), yy[idx2]], "k-"
                )  # line from last data point
                semilogy(
                    np.dot(dat.x[-2, iabscissa], [1, 1]),
        ...
        title(
            "Object Variables ("
            + ("mean" if plot_mean else "curr best")
            + ", "
            + str(dat.x.shape[1] - 5)
            + "-D, popsize~"
            + (
                str(int((dat.f[-1][1] - dat.f[-i][1]) / (dat.f[-1][0] - dat.f[-i][0])))
                if len(dat.f.T[0]) > 1 and dat.f[-1][0] > dat.f[-i][0]
                else "NA"
            )
            + ")"
        )
        ...
        if 1 < 2 and dat.std.shape[1] < 100:
            ...
            idx2 = np.argsort(idx)
            dat.std[-1, 5 + idx] = np.logspace(
                np.log10(np.min(dat.std[:, 5:])),
            ...
            # vertical separator
            plot(
                np.dot(dat.std[-2, iabscissa], [1, 1]),
    ...
    def plotdivers(dat, iabscissa, foffset):
        ...
        if 11 < 3:  # delta-fitness as points
            ...
            semilogy(
                dat.f[1:, iabscissa],  # abs(fit(g) - fit(g-1))
                np.abs(dfit) + foffset,
                ".c",
            )
            i = dfit > 0
            ...
            semilogy(
                dat.f[1:, iabscissa][i],  # abs(fit(g) - fit(g-1))
                np.abs(dfit[i]) + foffset,
                ".r",
            )
        ...
        text(
            ax[0] + 0.01,
            ax[2],  # 10**(log10(ax[2])+0.05*(log10(ax[3])-log10(ax[2]))),
    ...
    def disp(self, idx=100):  # r_[0:5,1e2:1e9:1e2,-10:0]):
        ...
        def printdatarow(dat, iteration):
            ...
            print(
                ...
                + " %.14e" % (dat.f[i, 5])
                + " %5.1e" % (dat.f[i, 3])
                + " %6.2e" % (max(dat.std[j, 5:]))
                + " %6.2e" % min(dat.std[j, 5:])
            )
...
class DEAPCMADataLogger(BaseDataLogger):  # might become a dict at some point
    ...
    def initialize(self, modulo=None):
        ...
        if 11 < 3:
            ...
            try:
                with open(fn, "w") as f:
                    f.write(
                        "% # iter+eval+sigma+0+fitness+xbest, " + strseedtime + "\n"
                    )
            except (IOError, OSError):
                print("could not open/write file " + fn)
    ...
    def add(
        self, fitness_values, es=None, more_data=[], modulo=None
    ):  # TODO: find a different way to communicate current x and f
        """append some logging data from `CMAEvolutionStrategy` class instance `es`,
        if ``number_of_times_called % modulo`` equals to zero, never if ``modulo==0``.
            ...
            with open(fn, "a") as f:
                f.write(
                    ...
                    + " ".join(
                        map(
                            str,
                            es.sigma * np.sqrt([es.C[i][i] for i in xrange(es.dim)]),
                        )
                    )
                    + "\n"
                )
...
class AII(object):
    ...
    def __init__(self, x0, sigma0, randn=np.random.randn):
        ...
        self.eta_r = (
            0 / N / 3
        )  # c_r learning rate for direction, cigar: 4/N/3 is optimal in 10-D, 10/N/3 still works (15 in 20-D) but not on the axparallel cigar with recombination
        self.mu = 1
        ...
        self.damps = 1
        self.use_sign = 0
        self.use_scalar_product = (
            0  # sometimes makes it somewhat worse on Rosenbrock, don't know why
        )
    ...
    def ask(self, popsize):
        ...
        pop = [
            self.mean
            + self.sigma * (self.sigmai * self.Z[k])
    ...
    try:  # pass on KeyboardInterrupt
        ...
        while 1:
            ...
            while not es.stop():
                ...
                if noise_handling:
                    es.sigma *= (
                        noisehandler(X, fit, func, es.ask, args)
                        ** opts["noise_change_sigma"]
                    )
                    es.countevals += (
                        noisehandler.evaluations_just_done
                ...
                if (
                    ...
                    and opts["verb_plot"]
                    and (
                        es.countiter % max(opts["verb_plot"], opts["verb_log"]) == 0
                        or es.stop()
                    )
                ):
            ...
            if opts["verb_disp"]:
                srestarts = (
                    (" after %i restart" + ("s" if irun > 1 else "")) % irun
                    if irun
                    else ""
                )
                for k, v in list(es.stop().items()):
                    print(
                        "termination on %s=%s%s (%s)"
                ...
                if es.N < 9:
                    ...
                    print(
                        "std deviation: " + str(es.sigma * sqrt(es.dC) * es.gp.scales)
                    )
                else:
                    print(
                        "mean solution: %s ...]" % (str(es.gp.pheno(es.mean)[:8])[:-1])
                    )
                    print(
                        "std deviations: %s ...]"
            ...
            if (
                irun > restarts
                or "ftarget" in es.stopdict
                or "maxfunevals" in es.stopdict
            ):
                break
        ...
        es.out["best"] = (
            best  # TODO: this is a rather suboptimal type for inspection in the shell
        )
        if 1 < 3:
...
def plot(
    ...
    """
    CMADataLogger(name).plot(
        fig, abscissa, iteridx, plot_mean, foffset, x_opt, fontsize
    )
...
class Sections(object):
    ...
    def __init__(
        ...
        basis=None,
        name=None,
        plot_cmd=pylab.plot if pylab else None,
        load=True,
    ):
        """
        Parameters
    ...
    def plot(self, plot_cmd=None, tf=lambda y: y):
        ...
        for i in sorted(res.keys()):  # we plot not all values here
            if type(i) is int:
                ...
                plot_cmd(
                    arx, [tf(np.median(res[i][x]) + addf) for x in arx], color + "-"
                )
...
class Misc(object):
    ...
    def likelihood(x, m=None, Cinv=None, sigma=1, detC=None):
        ...
        return (
            exp(-np.dot(dx, np.dot(Cinv, dx)) / sigma**2 / 2)
            / s2pi
            / abs(detC) ** 0.5
            / sigma**n
        )
...
class FitnessFunctions(object):
    ...
    def elli(self, x, rot=0, xoffset=0, cond=1e6, actuator_noise=0.0, both=False):
        ...
        beta = 1
        felli = (
            np.random.rand(1)[0] ** beta
            * ftrue
            * max(1, (10.0**9 / (ftrue + 1e-99)) ** (alpha * np.random.rand(1)[0]))
        )
    ...
    def ridge(self, x, expo=2):
        x = [x] if np.isscalar(x[0]) else x  # scalar into list
        f = [x[0] + 100 * np.sum(x[1:] ** 2) ** (expo / 2.0) for x in x]
        return f if len(f) > 1 else f[0]  # 1-element-list into scalar
    ...
    def griewank(self, x):
        # was in [-600 600]
        x = (600.0 / 5) * x
        return 1 - np.prod(np.cos(x / sqrt(1.0 + np.arange(len(x))))) + sum(x**2) / 4e3
...
def main(argv=None):
    ...
    if len(argv) >= 1:  # function and help
        if len(argv) == 1 or argv[1].startswith("-h") or argv[1].startswith("--help"):
            ...
            if len(argv) > 2 and (
                argv[2].startswith("--v") or argv[2].startswith("-v")
            ):  # verbose
                print(
