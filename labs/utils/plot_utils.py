...
from neat import Config, StatisticsReporter
...
def plot_observer(
    ...
    if args.get("class") is not None and isinstance(
        population[0].fitness, args["class"]
    ):
...
def plot_spikes(
    ...
    t_values, I_values, v_values, u_values, f_values = zip(*spikes, strict=False)
    ...
    if title is None:
        ...
        fig.suptitle(f"Izhikevich's spiking neuron model ({title!s})")
...
def plot_species(
    statistics: StatisticsReporter, view: bool = True, filename: str | None = None
) -> None:
...
def draw_net(
    ...
    view: bool = True,
    filename: str | None = None,
    node_names: dict[Any, Any] | None = None,
    show_disabled: bool = True,
    prune_unused: bool = False,
    node_colors: dict[Any, Any] | None = None,
    fmt: str = "png",
) -> Any:
...
def plotTrends(logbook: Logbook, name: str | None = None):
